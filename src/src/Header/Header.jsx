...
export function Header() {
  ...
  const handleSubmit = async (e, action) => {
    ...
    if (action === 'login') {
      try {
        const response =  await axios.post('http://localhost:3500/auth', JSON.stringify(userData), config);
      ...
      try {
        const response =  await axios.post('http://localhost:3500/register', JSON.stringify(userData), config);
      } catch (error) {
  ...
  const handleLogout = async (e, action) => {
    ...
    try {
      const response =  await axios.post('http://localhost:3500/logout', JSON.stringify(userData), config);
