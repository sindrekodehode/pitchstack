...
export function Header() {
  ...
  const handleSubmit = async (e, action) => {
    ...
    if (action === 'login') {
      try {
        const response =  await axios.post('https://20.223.156.203/auth', JSON.stringify(userData), config);
      ...
      try {
        const response =  await axios.post('https://20.223.156.203/register', JSON.stringify(userData), config);
      } catch (error) {
  ...
  const handleLogout = async (e, action) => {
    ...
    try {
      const response =  await axios.post('https://20.223.156.203/logout', JSON.stringify(userData), config);
