                    ...
                    {selectedPDFData.map((pdfData, index) => {
                        ...
                        <div key={index} className={styles.pdfcontainer}>
                            ...
                            {pdfData.data && Object.entries(pdfData.data).map(([key, value]) => (
                                <div key={key} className={[styles.itemCard, activeDiv === index && styles.active].filter(Boolean).join('')} onClick={() => clickDiv(key)} style={activeDiv === index ? {positon: 'absolute', zindex: 10, transform: 'translate(-50%, -50%) scale(1.5)', left: '50%', top: '50%'} : {}}>
