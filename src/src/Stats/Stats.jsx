                    ...
                    {selectedPDFData.map((pdfData, index) => {
                        ...
                        <div key={index} className={styles.pdfcontainer}>
                            ...
                            {pdfData.data && Object.entries(pdfData.data).map(([key, value]) => (
                                <div key={key} className={[styles.itemCard, activeDiv === index && styles.active].filter(Boolean).join('')} onClick={() => clickDiv(key)} style={activeDiv === key ? {positon: 'absolute', zindex: 10, transform: 'scale(1.5)', background: 'rbg(2 2 2 / 91%)', left: '40%', top: '10%'} : {}}>
